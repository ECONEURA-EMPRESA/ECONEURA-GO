...
const n8nWebhookRequestSchema = z.object({
...
export class N8NAdapter {
  ...
  async executeWebhook(request: N8NWebhookRequest): Promise<Result<N8NWebhookResponse, Error>> {
    try {
      const validated = n8nWebhookRequestSchema.parse(request);

      logger.info('[N8NAdapter] Ejecutando webhook', {
      ...
      if (axios.isAxiosError(e)) {
        logger.error('[N8NAdapter] Error HTTP ejecutando webhook', {
          error: error.message,
      ...
      logger.error('[N8NAdapter] Error ejecutando webhook', {
        error: error.message
      });
